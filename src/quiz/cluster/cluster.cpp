...
void proximity(int pointID, std::vector<int>& cluster, const std::vector<std::vector<float>>& points, KdTree* tree, std::vector<bool>& processedPoints, float distanceTol)
{
	processedPoints.at(pointID) = true;
	cluster.push_back(pointID);
	std::vector<int> nearbyPointsIDs = tree->search(points.at(pointID), distanceTol);
	for(int pointID:nearbyPointsIDs)
	{
		if (!processedPoints.at(pointID))
		{
			proximity(pointID, cluster, points, tree, processedPoints, distanceTol);
		}
	}
}
